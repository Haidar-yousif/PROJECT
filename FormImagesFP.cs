...
namespace PROJECT
{
    public partial class FormImagesFP : Form
    {
        int SerialNo;
        ...
        public FormImagesFP(int serp, int ser)
        {
            isvalid = 0;
            SerialNo = ser;
        ...
        private void FormImagesFP_FormClosed(object sender, FormClosedEventArgs e)
        {
            Close();
        }
        private void HandlePictureBox(PictureBox Box, ref byte[] bytelist)
        {
            try
            {
                ...
                {
                    openFileDialog.Title = "finger print image";
        ...
        private void pictureBox2_Click(object sender, EventArgs e)
        {
            if (!isExist())
            {
                HandlePictureBox(pictureBox2, ref right_image);
            }
            else
            {
                MessageBox.Show("there is record already");
        ...
        private bool isExist()
        {
            ...
            {
                var imagefps = context.ImageFps;

                bool isexist = context.ImageFps.Any(v => (v.Serial == SerialNo) && (serperFromPerson == v.Serpers));
        ...
        private void FillPictureBox()
        {
            using (var context = new InvEntitie())
            {
                ...
                var target = context.ImageFps.FirstOrDefault(v => (v.Serial == SerialNo) && (v.Serpers == serperFromPerson));
                if (target == null) return;
                pictureBox1.Image = ByteArrayToImage(target.Fpleft);
                pictureBox2.Image = ByteArrayToImage(target.Fpright);
        ...
        private void pictureBox1_DoubleClick(object sender, EventArgs e)
        {
            using (var context = new InvEntitie())
            {
                var imagefaces = context.ImageFps;
                bool isexist = context.ImageFps.Any(v => (v.Serial == SerialNo) && (serperFromPerson == v.Serpers));
                if (isexist)
                {
                    HandlePictureBox(pictureBox1, ref left_image);
                    ...
                    using (var context = new InvEntitie())
                    {
                        ...
                        if (isexist)
                        {
                            ...
                            if (MessageBox.Show("Are you need to update this images ? ", "open paint", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                                ...
                                if (left_image.Length != 0) target.Fpleft = left_image;
                                if (right_image.Length != 0) target.Fpright = right_image;


                                context.SaveChanges();

                                MessageBox.Show("opartion done successfuly");
                                FormImagesFP_Load(sender, e);
                            }
                            else
                            {
                                MessageBox.Show("operation was canceled");
        ...
        private void FormImagesFP_Load(object sender, EventArgs e)
        {
            Refresh();
            isvalid = 0;
